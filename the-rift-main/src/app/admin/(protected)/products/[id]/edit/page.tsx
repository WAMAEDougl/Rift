...
export default function EditProductPage() {
    ...
    Promise.all([
      ...
      if (prodJson.data) {
        ...
        setForm({
          ...
          nutrition_highlights: Array.isArray(p.nutrition_highlights) ? p.nutrition_highlights.join("\n") : "",
        });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) { toast.error(json.error?.message ?? "Failed to save product"); return; }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-3xl font-medium text-foreground">Edit Product</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
              ...
              <div>
                ...
                <textarea value={form.features} onChange={(e) => set("features", e.target.value)}
                  rows={4} placeholder="One feature per line" className={inputCls + " resize-none font-mono text-xs"} />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.nutrition_highlights} onChange={(e) => set("nutrition_highlights", e.target.value)}
                  rows={3} placeholder="One highlight per line" className={inputCls + " resize-none font-mono text-xs"} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
              ...
              {[
                { field: "in_stock", label: "In Stock", desc: "Available to purchase" },
                { field: "is_active", label: "Active / Visible", desc: "Show on storefront" },
