...
export const metadata: Metadata = {
  title: "Health Hub | Ayola Foods KE",
  description:
    ...
    <>
      ...
      <section className="pt-28 pb-16 bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-secondary text-sm font-semibold uppercase tracking-wider inline-flex items-center gap-2">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
            Understand the food science behind Ayola products. Written by food
            scientist Prisca Kiragu (JKUAT) — real knowledge, not marketing hype.
          </p>
        </div>
      </section>
            ...
            {topics.map((topic, i) => (
              ...
              >
                <div className={i % 2 === 1 ? "lg:order-2" : ""}>
                  <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${topic.color} text-white flex items-center justify-center mb-4`}>
                    {topic.icon}
                  </div>
                  <h2 className="text-3xl font-bold text-earth mb-3">{topic.title}</h2>
                  <p className="text-muted-foreground leading-relaxed mb-6">{topic.description}</p>
                  ...
                  <p className="mt-4 text-sm text-muted-foreground/60">
                    Try it: <span className="text-primary font-semibold">{topic.ayolaProduct}</span>
                  </p>
                </div>
                <div className={`bg-gradient-to-br ${topic.color} rounded-3xl p-12 flex items-center justify-center aspect-square max-h-80 ${i % 2 === 1 ? "lg:order-1" : ""}`}>
      ...
      <section className="py-16 bg-muted/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-earth mb-3">Dietary Guides</h2>
            <p className="text-muted-foreground">Find the right Ayola products for your health goals.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {dietaryGuides.map((guide) => (
              <div key={guide.title} className="bg-card rounded-2xl border border-border p-6">
                <h3 className="font-bold text-foreground mb-1">{guide.title}</h3>
                <p className="text-sm text-muted-foreground mb-4">{guide.description}</p>
                <ul className="space-y-1.5">
                  {guide.products.map((p) => (
                    <li key={p} className="text-sm text-primary font-medium">• {p}</li>
                  ))}
                </ul>
              </div>
            ))}
      ...
      {/* CTA */}
      <section className="py-16 bg-gradient-to-r from-secondary to-secondary-light text-white text-center">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold mb-4">Ready to Eat Healthier?</h2>
          <p className="text-white/80 mb-8 text-lg">
            Every Ayola product is designed with your health in mind. Start your journey today.
          </p>
          <div className="flex justify-center gap-4 flex-wrap">
            <Link href="/products" className="inline-flex items-center gap-2 bg-card text-secondary px-8 py-3.5 rounded-xl font-bold hover:bg-card/90 transition-colors">
              Shop Products <ArrowRight className="w-4 h-4" />
            </Link>
            <Link href="/recipes" className="inline-flex items-center gap-2 border-2 border-white text-white px-8 py-3.5 rounded-xl font-bold hover:bg-card/10 transition-colors">
              Watch Recipes
            </Link>
          </div>
        </div>
